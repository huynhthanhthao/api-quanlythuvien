...
function checkToken(req, res, next) {
  ...
  jwt
    .verify(token, ACCESS_TOKEN_SECRET, async (err, decode) => {
      if (err) {
      ...
      if (!account || account?.status == ACCOUNT_STATUS.BLOCKED) throw new CatchException("Tài khoản đã bị khóa!", errorCodes.FORBIDDEN);
