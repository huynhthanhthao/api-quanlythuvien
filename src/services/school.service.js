...
const { SECRET_KEY } = require("../../enums/setup");

class SchoolService {
  static async createSchool(newSchool) {
    if (newSchool.secretKey != SECRET_KEY) throw new CatchException("Secret key không đúng!", errorCodes.INVALID_DATA);
  ...
  static async updateEmailSMTP(emailSMTPUpdate, account) {
    ...
    await db.SchoolEmailSMTP.update({ email: emailSMTPUpdate.email, password: emailSMTPUpdate.password }, { where: { ...whereCondition, id: school.schoolEmailSMTPId } });
  }
