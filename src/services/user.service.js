...
class UserService {
  ...
  static async getUsers(query, account) {
    ...
    const whereClassCondition = {
      [Op.and]: [classIds.length > 0 && { id: { [Op.in]: classIds } }, { schoolId: account.schoolId }, { active: true }].filter(Boolean),
    };
  ...
  static async extendUser(userExtend, account) {
    ...
    if (user.type != USER_TYPE.READER) throw new CatchException("Người dùng không phải là bạn đọc!", errorCodes.INVALID_DATA, {});
  ...
  static async checkUserValidity(id, account, field) {
    ...
    if (endDay && getStartOfDay(endDay) / 1000 < getStartOfDay(new Date()) / 1000) throw new CatchException("Bạn đọc này đã hết hạn!", errorCodes.RESOURCE_HAS_EXPIRED, { field: field });
  }
  ...
  static async getUserByToken(token) {
    ...
    await jwt.verify(token, ACCESS_TOKEN_SECRET, async (err, decode) => {
      if (err) {
